...
Written by Nick Parlante, Sonja Johnson-Yu, Nick Bowman, and Parth Sarin.
 -7/2019  version, has file reading, pix, foreach, hidden get/setpix
 -5/2020  version, has text options
...
class SimpleImage(object):
    ...
    def _text_wrap(self, text, font, max_width):
        """
        Wraps text when it reaches max width.
        ...
        if font.getsize(text)[0] <= max_width:
            ...
            words = text.split(' ')
            i = 0
            # append every word to a line while its width is shorter than the max
            while i < len(words):
                line = ''
    ...
    def create_text(self, text, x, y, font, size=None, color='rgb(0, 0, 0)'):
        """
        ...
        font (str) -- The filename of the font to draw the text in.
        size (int or None) -- Integer font or defaults to 15% of image height.
        color (str) -- Color for the text or defaults to black.
        """
        # Initialize optional parameters
        size = size or round(self._height * .15)
        width_buffer = self._width - x

        # PIL controllers
        draw = ImageDraw.Draw(self.pil_image)
        font = ImageFont.truetype(font, size=size, encoding='unic')

        # Wrap text and draw
        lines = self._text_wrap(text, font, width_buffer)
